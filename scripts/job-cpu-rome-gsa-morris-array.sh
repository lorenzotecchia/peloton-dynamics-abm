#!/usr/bin/env bash
# Run the Morris GSA as a Slurm ARRAY: split the design matrix into K chunks, each
...
#   merge job   : `--mode merge` concatenates the Y chunks -> gsa_morris.csv
#                 (Morris analysis needs X too; merge reads it from WORK_DIR).
#
# Usage (from the project root, on a login node; run with bash, NOT sbatch):
#   bash scripts/job-cpu-rome-gsa-morris-array.sh [SAMPLES] [REPLICATES] [GENERATIONS] \
#                                                 [MAX_STEPS] [K] [ROAD_LENGTH] [DT] [GROUP_RADIUS]
# Defaults: SAMPLES=20, REPLICATES=10, GENERATIONS=150, MAX_STEPS=2500, K=4,
...
#                TASK_TIME (per-chunk wall time, default 08:00:00).
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
    ...
    echo "  bash scripts/job-cpu-rome-gsa-morris-array.sh [SAMPLES] ..." >&2
    exit 1
fi
...
GENERATIONS="${3:-150}"
MAX_STEPS="${4:-2500}"
K="${5:-4}"                 # number of array chunks (= nodes used in parallel)
ROAD_LENGTH="${6:-10000}"
DT="${7:-2}"
GROUP_RADIUS="${8:-3}"
GSA_OUT_BASE="${GSA_OUT_BASE:-/gpfs/work5/0/prjs2142/gsa-agent-dump-per-run}"
TASK_TIME="${TASK_TIME:-08:00:00}"
