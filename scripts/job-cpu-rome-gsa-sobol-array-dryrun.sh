#!/usr/bin/env bash
# DRY RUN of the Sobol GSA ARRAY pipeline: same split sample->evaluate->merge flow
# as job-cpu-rome-gsa-sobol-array.sh, but with the dry-run amount (SAMPLES=128,
...
#   bash scripts/job-cpu-rome-gsa-sobol-array-dryrun.sh [SAMPLES] [REPLICATES] [GENERATIONS] \
...
# Output: <GSA_OUT_BASE>/<ARRAY JOB ID>-<GIT HASH>-sobol/gsa_sobol.csv (+ _S2, sample_index).

set -euo pipefail

if [[ -n "${SLURM_JOB_ID:-}" ]]; then
    echo "ERROR: launch with 'bash', not 'sbatch' (this wrapper submits the jobs itself):" >&2
    echo "  bash scripts/job-cpu-rome-gsa-sobol-array-dryrun.sh [SAMPLES] ..." >&2
    exit 1
fi
