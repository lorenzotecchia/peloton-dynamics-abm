...
class PelotonConfig:
    ...
    road_length: float = 100000.0  # finish line position (x_max)
    ...
    dt: float = 60.0  # seconds of race per model step
    ...
    breakaway_speed_frac: float = (
        0.95  # solo speed of a breakaway = frac * s_m (Hoenigman)
    )
    breakaway_cooldown_steps: int = (
        10  # steps during which recent breakers are kept separate from original packs
    )

    # --- Across-race evolution. ---
    utility_decay: float = (
        0.4  # lambda: exp decay of position-based utility (larger = steeper)
    )
