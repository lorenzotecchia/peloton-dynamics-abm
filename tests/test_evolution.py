...
def test_evolve_pulls_loser_toward_similar_winner():
    ...
    assert 0.0 < loser.coeffs["effort"]["alpha"] < 2.0
    assert winner.coeffs["effort"]["alpha"] == 2.0      # nobody better -> unchanged
...
def test_run_generations_runs_and_returns_history():
    ...
    history, _ = evolution.run_generations(n_generations=3, max_steps=80, config=cfg)
...
def test_run_generations_records_coefficient_trajectories():
    ...
    # Gen 0 is the initial population (each rider samples default_coeffs), so the
    # spread is positive but finite before any learning.
